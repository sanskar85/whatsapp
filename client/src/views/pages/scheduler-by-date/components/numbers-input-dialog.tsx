...
import { setRecipientsData as setPrefilledNumbers } from '../../../../store/reducers/SchedulerByDateReducer';
...
export default function NumberInputDialog({ isOpen, onClose }: Props) {
	const {
		details: { recipient_data: prefilledNumbers },
	...
	useEffect(() => {
		if (!prefilledNumbers || typeof prefilledNumbers === 'string' || !Array.isArray) return;
		setNumberInput((prefilledNumbers as string[]).join(', '));
	}, [prefilledNumbers]);
